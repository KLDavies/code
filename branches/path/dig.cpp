...
static int process_dir(state *s, TCHAR *fn)
{
  ...
  // RBF - Need higher limit for Win32
  new_file = (TCHAR *)malloc(sizeof(TCHAR) * 32767);
  if (NULL == new_file)
...
#ifdef _WIN32
    return_value = process_win32(s,new_file);
#else
...
int process_win32(state *s, TCHAR *fn)
{
  ...
  TCHAR *dirname, *extended_fn, new_fn[32767], *tmp;
  ...
  extended_fn = (TCHAR *)malloc(sizeof(TCHAR) * (_tcslen(fn) + 5));
  if (_tcsnicmp(fn, _TEXT("\\\\?\\"), 4))
  {
    _sntprintf(extended_fn, 32767, _TEXT("%s%s"), _TEXT("\\\\?\\"), fn);
  }
  else
  {
    _tcscpy(extended_fn, fn);
  }

  hFind = FindFirstFile(extended_fn, &FindFileData);
...
#define FATAL_ERROR_MEM(A) if (NULL == A) fatal_error("%s: Out of memory", __progname);
  ...
  // The first entry returned can be the original filename we specified
  // We need the directory name, not the original filename
  dirname = _tcsdup(extended_fn);
  FATAL_ERROR_MEM(dirname);
  my_dirname(dirname);

  do
  {
    print_error_unicode(s, FindFileData.cFileName, "<-- found file");
	...
	_sntprintf(new_fn, 
      ...
      {	  
	MD5DEEP_ALLOC(TCHAR, tmp, 32767);
	_sntprintf(tmp, 
		   32767, 
		   _TEXT("%s\\%s"), 
		   dirname,
		   FindFileData.cFileName);
	_wfullpath(new_fn, tmp, 32767);
	free(tmp);
      }

      print_error_unicode(s,new_fn,"<-- the new fn");
    
      if (FindFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
      {
	if (s->mode & mode_recursive) 
	  process_dir(s, new_fn);
	else
